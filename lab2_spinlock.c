...
#define RUNNING 0
#define DONE 1
#define NEXTSTEP 2 //next multiple in integer array 
#define PERLINE 8 //number of elements printed per line
...
//define locks used in barrier synchronization and prime computation
DEFINE_SPINLOCK(lock1);
DEFINE_SPINLOCK(lock2);
DEFINE_MUTEX(cur_lock); 
DEFINE_MUTEX(cross_lock);
...
sieve (unsigned long *counter) 
{
	...
	while(1) 
    {
		...
		cur_pos = pos;
        //increment position for ensuing threads
		pos++;
		mutex_unlock(&cur_lock);
        //crossing out multiples of primes
		for(i=cur_pos * NEXTSTEP;i<=upper_bound;i+=cur_pos)
        {
            mutex_lock(&cross_lock);
			nums[i] = 0;
			(*counter)++;
            mutex_unlock(&cross_lock);
		}
        schedule();
	}
}
...
print_stats (void)
{
  ...
  d1 = ts_1 - ts_init;
  //module runtime = barrier_2 time - barrier_1 time = sieve computation time
  d2 = ts_2 - ts_1;
